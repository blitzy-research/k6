...
|---------------------|---------------------------|------------------|:------------:|---------------------------------------------------|
| `blocked` | `tracer.go:L323-L325` | `gotConn − getConn`: connection-acquisition time (DNS + TCP + TLS + pool wait) | **Yes** | **Bimodal**: cold ≈ 2.25–3.09 ms, warm ≈ 0.002–0.012 ms (§3.2). Near-zero on idle-pool checkout is correct. `0` if the guard `gotConn>getConn` is not met (e.g. dial-refused, §3.3). |
