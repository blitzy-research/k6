...
**Q4 — Empirical proof.**
A module seen at init **can** be re-`require()`'d later by VUs (DEMO 1, exit `0`); a never-seen module **reliably fails** with the frozen-resolver error (DEMO 2, exit `107`). The same `"./shared.js"` resolves to two different files depending on the caller (DEMO 3, exit `0`). All exact error/warning strings and exit codes are in §6–§8 (the demo sections below: DEMO 1–5 in §6, DEMO 6–7 in §7, DEMO 8 in §8).
...
Static ESM `import` declarations are resolved during init, so the imported modules enter the shared cache before `Lock()`. A *runtime dynamic* `import()` expression, however, is **not wired up in the standard k6 host program**. k6's JavaScript engine is the Sobek (goja-fork) runtime `[go.mod:L78]`, and Sobek gates runtime dynamic `import()` behind a host-registered callback — `Runtime.SetImportModuleDynamically(callback ImportModuleDynamicallyCallback)` `[vendor/github.com/grafana/sobek/modules.go:L461,L463-L465]`. The k6 host program **never registers that callback**: there is no call to `SetImportModuleDynamically` anywhere in the non-vendored k6 source (`js/`, `cmd/`, `lib/`, `loader/`), so the dynamic-`import()` handler is left unset and the dynamic-loading path that users actually reach at runtime is `require()`. That is why the freeze is observed through `require()` rather than through `import()`. (The k6 maintainers themselves note, in the design discussion behind this restriction, that dynamic `import()` would face the same init-context constraints as `require()` — see grafana/k6 issue #3020, cited in §9.)
...
- **Built-ins** are keyed by their **literal name** — `"k6"`, `"k6/http"`, `"k6/x/..."`. The built-in branch checks the cache by that literal name and, on a miss during the first init, populates `mr.cache[arg]` with the resolved module `[js/modules/resolution.go:L147-L155]`.
...
The freeze, and the broader requirement that all module imports and file opens happen during init, exist to enable **archiving and distributed execution**. k6 can bundle a test into a self-contained archive via the `k6 archive` sub-command `[cmd/archive.go:L92-L97]` — whose own command description reads *"An archive is a fully self-contained test run, and can be executed identically elsewhere"* `[cmd/archive.go:L97]` — built by `Runner.MakeArchive()` `[cmd/archive.go:L35]`. For that archive to be complete (and for the same test to run identically across distributed instances — k6 cloud / distributed runs), k6 must know *every* module and file the test will ever touch **up front**, during init, so it can collect and ship them. If a VU could pull in an arbitrary new module mid-test, the archive could be incomplete and a distributed run could diverge.

This rationale comes directly from the k6 maintainers. The design issue **grafana/k6 #3020** — "Let `open` and `require` be used outside of the init context, somewhat" (<https://github.com/grafana/k6/issues/3020>) — explains that `open`/`require` are confined to the init context specifically so that `k6 archive` (and, by extension, k6 cloud and any future distributed run) can gather every file the test needs by running init up front; the same issue explicitly notes that dynamic `import()` would face the same constraint as `require()`, which is the basis for §3(c).
...
This is a **deliberate design property**, not a performance optimization that kicks in under load. The official k6 documentation corroborates the same model: the [Test lifecycle](https://grafana.com/docs/k6/latest/using-k6/test-lifecycle/) docs state that init code *"runs once per VU"* to prepare the test (loading files and importing modules), and the [Running k6](https://grafana.com/docs/k6/latest/get-started/running-k6/) docs document the three execution modes that the archive ultimately enables — local, distributed, and cloud. The code remains the source of truth for the *mechanics*; these references corroborate the *intent*.
