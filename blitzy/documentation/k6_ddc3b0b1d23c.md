...
| Item | Value |
|------|-------|
| k6 version | `v0.55.0` — the value of the `Version` constant [lib/consts/consts.go:L12], confirmed by the built binary's `version` output. **Analyzed source:** the `k6_ddc3b0b1d23c` source branch, whose stable base commit is `ddc3b0b1d` (encoded in the branch name and this document's filename). k6 derives the `commit/<hash>` suffix of its version string from the git HEAD (`vcs.revision`, truncated to 10 characters) at *build* time [lib/consts/consts.go:L30-L52], so that suffix reflects whatever HEAD happens to be — including documentation-only commits on this branch — and is therefore **not** a stable identifier of the analyzed source. We cite the fixed base commit `ddc3b0b1d` rather than a volatile per-build hash. |
...
**Empirical evidence.** The repository encodes exactly this in `TestRampingVUsHandleRemainingVUs` [lib/executor/ramping_vus_test.go:L311]: of two VUs at ramp-down, it expects one to *finish* its iteration and one to be *interrupted* once the graceful window closes [lib/executor/ramping_vus_test.go:L370-L371] — the precise `gracefulStop`/`hardStop` boundary described above. (That assertion is timing-sensitive: under the full-package `-race` suite at default parallelism it is flaky and can fail on the finish-vs-interrupt timing — never on a data race — while passing reliably in isolation and under constrained parallelism (`-parallel 1`) — with or without `-race`. See § 7.4.)
...
**Full package race suite (race-gated).** The project's own zero-tolerance gate is `go test -race -timeout 210s ./...` [Makefile:L29]; the executor slice of it is `go test -race ./lib/executor/...`. Run **bare, at the default parallelism of a multi-core machine, this command is timing-flaky and can fail** (`exit 1`, package time ~30s) — *not* because of any data race, but because the pre-existing timing-sensitive `TestRampingVUsHandleRemainingVUs` (see the note below) intermittently skews. In this investigation the bare command **failed 1 of 3 consecutive runs** on a 4-CPU host (the other two passed, each in ~30s); on some multi-core environments it fails **reliably** on every bare run. Crucially, the package reports **zero data races whether it passes or fails**. Reliable green runs are obtained by removing the *parallel* timing contention — never by any code change — and the determining factor is **reduced parallelism, not the absence of the detector**: **(i) in isolation**, `go test -run '^TestRampingVUsHandleRemainingVUs$' -race -count=10 ./lib/executor/` passes **10/10** with zero races; **(ii) with constrained parallelism** — `-parallel 1`, which serializes the package's many `t.Parallel()` tests — the *full* package passes reliably **whether or not `-race` is enabled**: `GOMAXPROCS=1 go test -race -parallel 1 ./lib/executor/` in **~108s** with zero races, and the same command without `-race` in **~99s**. Removing `-race` *by itself* does **not** stabilize it: at default parallelism the fast bare run stays timing-flaky in *both* modes — with `-race` (~30s; failed 1 of 3 bare runs here) and, just as much, without `-race` (~22s; which failed **all 5** bare runs on this 4-CPU host) — because the skew comes from the `t.Parallel()` tests contending for cores, not from the detector. The figure that matters for the central question is invariant across all of these: across **every** race-enabled run in this investigation — the scenario binaries above, the full executor-package suite (passing *and* failing alike), and the `cmd/tests` harness — the `-race` detector reported **zero data races, without exception**, and `cmd/tests` reported **zero goroutine leaks**. Those two invariants are the authoritative answer to the central question; the headline command's *exit code* is governed by a millisecond timing assertion, not by the race detector.

**A secondary, non-blocking timing note** (it never indicates a data race or a concurrency defect, and so does not change the race verdict): one test, `TestRampingVUsHandleRemainingVUs` [lib/executor/ramping_vus_test.go:L311], asserts on **millisecond-level** interrupt-vs-finish timing (10ms/40ms stages, a 30ms `gracefulRampDown`, a 50ms `gracefulStop`, and 65ms VU sleeps); it expects **exactly one VU to finish and one to be interrupted** [lib/executor/ramping_vus_test.go:L370-L371]. Under the `-race` detector's runtime slowdown combined with CPU contention from the package's many `t.Parallel()` race-instrumented tests, those tight windows skew so that the finish/interrupt split departs from the expected one-and-one — both VUs landing on the **same** side (either both finishing *or* both interrupted, depending on which way the timing tips on a given host). This makes the bare full-package `-race` command **genuinely flaky under default parallelism on multi-core machines**, *not* merely occasionally skewed: in this investigation it failed 1 of 3 bare runs (the failing run observed *both* VUs finishing — 2 finished / 0 interrupted), and other multi-core environments see it fail on every bare run. When it fails it is a **timing assertion, never a data race** — the detector still reports zero races. The test's own source comment acknowledges the fragility: the graceful budget was widened "to prevent the test to become flaky" [lib/executor/ramping_vus_test.go:L327-L328]. It passes reliably once the *parallel* timing contention is removed — **10/10 with `-race` in isolation**, and, for the full package, under constrained parallelism (`-parallel 1`) **whether or not `-race` is enabled** (~108s with `-race`, ~99s without); removing `-race` *alone*, at default parallelism, does **not** stabilize it — the fast bare run still skews in either mode (with `-race` ~30s, without `-race` ~22s). It is a pre-existing test-timing artifact, not a concurrency defect in the executor, and is out of scope to change under this analysis-only task.
...
- **Builds:** both succeeded (`exit 0`); the binaries report version `k6 v0.55.0` (the `Version` constant [lib/consts/consts.go:L12]). The trailing `commit/<hash>` they also print is the git HEAD short-hash embedded at build time [lib/consts/consts.go:L30-L52], which advances with every commit on the branch — including this analysis's own documentation commits — so it is deliberately **not** reproduced here as a fixed value. The stable identifier of the analyzed source is the `k6_ddc3b0b1d23c` branch's base commit `ddc3b0b1d`.
...
- **`go test -race ./lib/executor/...`:** run **bare at default parallelism this command is timing-flaky and can fail** (`exit 1`, ~30s) — it failed 1 of 3 consecutive bare runs here (the other two passed, each ~30s), and fails reliably on some multi-core hosts — owing to the pre-existing millisecond-timing test `TestRampingVUsHandleRemainingVUs`, **not** a data race. It is reliably green once the *parallel* timing contention is removed — the determining factor is reduced parallelism, not the detector: **10/10 in isolation** (`-run '^TestRampingVUsHandleRemainingVUs$' -race -count=10`), and the full package passes under constrained parallelism (`-parallel 1`) **both with `-race` (~108s) and without `-race` (~99s)**. Removing `-race` alone does not help — the fast bare run at default parallelism stays flaky in both modes (with `-race` ~30s, failed 1 of 3 here; without `-race` ~22s, failed all 5 bare runs here), never on a data race. The dedicated race tests `TestVUHandleRace` and `TestVUHandleStartStopRace`, plus the execution-plan and segment-tuple tests, all pass under `-race`. Decisively, the race detector reported **zero data races in every run — passing and failing alike, without exception** — and the `cmd/tests` harness reported **zero goroutine leaks**; those are the results that substantiate the no-race/no-leak verdict. (See § 7.4 for the full breakdown of this timing artifact.)
