...
**Reproducing the commit stamp (and why a build of the delivered checkout differs).** Because `FullVersion()` embeds the **live** `HEAD` of whatever checkout is built (the first 10 characters of `vcs.revision`, `lib/consts/consts.go:31`), the `commit/…` fragment is a function of the current git `HEAD`, not a fixed literal. The canonical `commit/ddc3b0b1d2` shown above was produced from the **source commit under test** (`ddc3b0b1d`). Building instead from the **delivered checkout** — which necessarily carries the documentation commit(s) *on top of* the source commit — stamps whichever documentation commit is currently at `HEAD` (again the first 10 characters of `vcs.revision`), so its `commit/…` fragment is that documentation commit's hash rather than `ddc3b0b1d2`, and it changes as documentation commits are added or amended. For example, a build of the integrated checkout observed during this investigation stamped `k6 v0.55.0 (commit/c7469bfb3a, go1.23.12, linux/amd64)` (an earlier documentation commit stamped `commit/b000496a59`) — neither of which is `commit/ddc3b0b1d2`. That is expected, not a discrepancy: the version string always reflects the built checkout's `HEAD`. To reproduce the canonical `commit/ddc3b0b1d2` stamp exactly, build from a **detached checkout of the source commit** in a real clone. (Go's VCS stamping needs a real `.git` **directory**, so a linked `git worktree` — whose `.git` is only a pointer file — does **not** stamp the commit; use a clone + `checkout`.) **[OBSERVED, exit 0]** (`<local-k6-checkout>` = the path to your local k6 repository):
...
**A user-defined `handleSummary()` can emit arbitrary files (OBSERVED).** Instead of (or in addition to) `--summary-export`, a script may export a `handleSummary(data)` function. k6 looks it up by the exported name `handleSummary` (`HandleSummaryFn = "handleSummary"`, `lib/consts/js.go:9`), retrieves it with `vu.getExported(consts.HandleSummaryFn)` and requires it to be callable via `sobek.AssertFunction` (`js/runner.go:377,379`; a non-function is rejected at `:381`). Its **return value is an object whose keys are output paths**: `HandleSummary` is invoked at `cmd/run.go:195`, and `handleSummaryResult` (`cmd/run.go:508`) iterates the returned keys (`for path, value := range result` at `cmd/run.go:522`), opening each non-`stdout`/`stderr` path with `fs.OpenFile(path, syscall.O_WRONLY|syscall.O_CREAT|syscall.O_TRUNC, 0o666)` (`cmd/run.go:518`) and writing its content. The script below returns one such key — an absolute `/tmp` path — so k6 writes exactly that file:
